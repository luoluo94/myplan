...
public class DbGenerator
{
    ...
    {
        Prop p = PropKit.use("SqlConfig.properties");
        C3p0Plugin c3p0Plugin = new C3p0Plugin(p.get("url"), p.get("user"), p.get("password"));
        c3p0Plugin.start();
    ...
    {
        String baseModelPackageName = "com.guima.domain";
        // base model 文件保存路径
        String baseModelOutputDir = PathKit.getWebRootPath() + "/src/main/java/com/guima/domain";

        // model 所使用的包名 (MappingKit 默认使用的包名)
        String modelPackageName = "com.guima.domain";
        // model 文件保存路径 (MappingKit 与 DataDictionary 文件默认保存路径)
        String modelOutputDir = baseModelOutputDir + "/../domain";
        ...
        ,"plan_create_num","admin_exception_record","user_record","user_login_record");
